...
| Mutate ops | partial | createChat/renameChat/setChatArchived/setChatNote/deleteChat/clearArchivedChats/renameDevice done (`clearArchivedChats` is desktop-only today — Settings → Archived); markChatSeen accepted as a no-op (unseen markers UI-local); `SetChatConfig` exists on the doc layer but is not yet exposed as a Mutate op. |
