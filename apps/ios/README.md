...
| Chat Note hover card + its menu items | Note Card on a long press: a `.contextMenu` preview 324pt wide, its height computed before the press at the reader's text size, holding its 12pt corner with a 14pt veil of the page colour as the menu's dim leaves it (`NoteCard.swift`) |
