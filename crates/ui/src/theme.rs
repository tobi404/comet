...
mod tests {
    ...
    fn dump_note_bar_contrast() {
        ...
        // sidebar and nothing in the product depends on reading it.
        //
        // Re-pinned after the upstream theme registry landed: `glass()` no
        // longer tints its surface, so frost now measures exactly what plain
        // surface does and every frosted row shifted down a little. The slot
        // colours themselves did not move. Dark stays above 4.5 everywhere;
        // light stays inside the band already accepted above.
        for (appearance, expected) in [
            (Appearance::Dark, [5.93, 5.93, 4.65, 4.65]),
            (Appearance::Light, [2.81, 2.81, 2.50, 2.50]),
        ] {
